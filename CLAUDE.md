...
- `config` — print the resolved config with the layer that set each value, per `configuration.md`. The registry path is not among them: `-c` remains the only override, because `LoadRepos`' own comment rules out a config key naming it
...
- Installed via `go install github.com/datapointchris/forge/v6@latest` or dotfiles `go-tools.sh`
