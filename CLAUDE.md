...
- `config` — loads the repo registry, and forge's own config:
  ...
  - **`toolchain`** on a repo entry declares its build surface: `components` (a `stack` plus the `dir` it lives in) and `sql_dialect`. Declared, never detected — the portfolio has five conventions for where a Go service lives (`api/`, `cli/`, root, and two legacy shapes), and a fact like the SQL dialect is not derivable from a layout at any level of tidiness. A repo can hold several components of one stack: an `api/` and a `cli/` that are both Go modules, deliberately isolated. Every target comes from the registry, resolved by name through `runner.SelectRepos`; nothing resolves a working directory to an entry, so a generator acts on what it was told rather than on where it was run.
...
- Python scripts: ruff-format, ruff-check and mypy a second time, selected by path rather than by tag. pre-commit picks Python files through identify, which reads the extension or the interpreter the shebang names — an extensionless app whose shebang is `#!/usr/bin/env -S uv run --script` names `uv`, so it gets no `python` tag and all three hooks above skip it without a word. The mypy half fails twice over: that hook runs `mypy .` with `pass_filenames: false`, and mypy collects a directory by extension, so fixing the tagging alone leaves it unreachable. `--scripts-are-modules` is what lets a commit touch two of these apps at once; without it mypy names every file it is handed directly `__main__` and stops on the collision. The paths come from a scan of the repo's tracked files, so a repo the scan finds nothing in generates byte for byte what it did before the block existed
...
**Every one carries `# forge-managed` on its first line, and that is what lets forge tell its own output from a file a person wrote.** `handWritten` reads it, and the die reports a file at a managed path that carries no marker rather than overwriting it. A separate marker from `# forge-toolchain: N` because the two answer different questions: a generated file's content is derived from the manifest and the version in its stamp is what a staged rollout reads, while a tool config is a static template and a version there would report eight files per repo as drift on every manifest bump that changed none of them. `TestEveryDeployedToolConfigCarriesTheManagedMarker` is what stops a ninth config arriving without one.

That marker is why **markdownlint and prettier are configured in YAML, not JSON**. JSON has no comment syntax, and a marker key is not a substitute: markdownlint ignores an unknown top-level key silently, but prettier honors the config and prints `Ignored unknown option` to stderr on every run. Both tools read the YAML spelling, including the markdownlint rev the markdown block pins.

**Both tools prefer the JSON spelling when both files are present**, so a repo keeping one has forge's config sitting unread while the old file governs. `toolConfig.supersedes` is what closes that: the old spelling is removed when its parsed content equals what forge writes at the new path, and reported when it differs. The removal does not claim forge wrote the file — it claims the content is reproduced at the new path in the same run, so nothing is lost. Writes are emitted before retractions, so a run interrupted between them leaves the old config still in force rather than a repo with no config for that tool.

- `markdownlint.yml` — deployed as `.markdownlint.yaml` to all repos, superseding `.markdownlint.json`
...
- `prettierrc.yml` — deployed as `.prettierrc.yaml` to Vue repos, superseding `.prettierrc.json`
...
A block must appear in `categoryMap` (stack-gated) or `genericBlocks` (every target); one in neither is an error rather than a silent default. Three categories are gated by something other than a component, and each is seeded by `Generate` rather than by `dirsByCategory`. `sql` follows a declared `sql_dialect` and fills `{{dialect}}`, because `.sql` files have no build directory of their own and nothing in them says postgres rather than sqlite. `git` holds conventional-commits and follows the `versioned` argument: it hooks a commit stage, so on a target git does not version it would be a hook that can never fire — and nothing would report it, since `uninstalledHooks` finds no missing stages where there is no `.git` to install them into. `python-scripts` follows the scan, fills `{{scripts}}` with an anchored alternation of the paths it found, and is left out entirely when it finds none; pre-commit reports a hook whose `files:` matched nothing as passing, which reads as coverage.

That scan is detection, against the rule above and for the reason `ReleaseGatesOnValidate` detects its own trigger. The failure being prevented is a new app landing beside the existing ones and nobody remembering to declare it, which is the same silent gap the block closes — a registry key cannot prevent that, and reading a first line cannot get it wrong. It re-derives on every generate, so adding or renaming an app shows up as an ordinary pending change. git's index is the enumeration, so a maintained directory is scanned not at all: it has no bounded one, and walking a home directory on every `check` is not a cost this is worth.
...
reports converged. The stamp is what authorizes that delete, so a hand-written config at either
...
not authorize installing a hook forge never installed, and dropping the measurement instead let
