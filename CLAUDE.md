...
- `shellcheckrc.ini` — deployed as `.shellcheckrc` to all repos, for the same reason as `.editorconfig`: the shell block is generic. Both settings supply information rather than removing findings — `external-sources` follows a sourced file instead of skipping it, and `source-path=SCRIPTDIR` resolves a `source=` directive against the sourcing file so a lint result does not depend on the working directory. **Never add a `disable=` line**: the config this replaced turned off SC2155 fleetwide and hid six real findings per repo, where `local x=$(cmd)` masks the command exit status. A genuinely unfollowable source takes a per-line `# shellcheck source=/dev/null` at the site instead
